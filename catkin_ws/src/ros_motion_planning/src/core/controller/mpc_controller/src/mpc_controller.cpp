...
ROS_INFO("[MPC] max_v = %.3f, min_v = %.3f, max_w = %.3f", max_v_, min_v_, max_w_);
...
bool MPCController::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
  ...
  // 当前速度的绝对值
  double v_abs = std::hypot(base_odom.twist.twist.linear.x, base_odom.twist.twist.linear.y);
  double wt = base_odom.twist.twist.angular.z;

  // 用绝对速度算前瞻距离（看多远和正倒无关）
  double L = getLookAheadDistance(v_abs);
  ...
  // 计算目标点相对机器人位置
  double dx = lookahead_pt.point.x - robot_pose_map.pose.position.x;
  double dy = lookahead_pt.point.y - robot_pose_map.pose.position.y;

  // 机器人朝向的单位向量
  double heading_x = std::cos(theta);
  double heading_y = std::sin(theta);

  // 点在车头前面(dot>0)还是车屁股后面(dot<0)
  ...
  else
  {
    ...
    // 参考控制：线速度带符号，角速度 = v * 曲率
    Eigen::Vector2d u_r(vt, regularizeAngle(vt * kappa));
