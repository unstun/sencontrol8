...
void HybridMotionTable::initReedsShepp(unsigned int & size_x_in,
                                       unsigned int & /*size_y_in*/,
                                       HybridSearchInfo & search_info)
{
  // 如果地图大小、角度数量、最小转弯半径都没变，并且当前已经是 Reeds–Shepp，就不用重新计算
  ...
  // 计算一个合适的转角（和 Dubins 一样的套路）：
  // 1. 是角度离散 bin 的整数倍
  // 2. 一步能跨出当前栅格（弦长 > sqrt(2)）
  ...
  // 使用 Reeds–Shepp 曲线生成器（支持前进 + 倒车）
  curve_gen = std::make_shared<ReedsSheppCurve>(
      search_info.curve_sample_ratio,
      1.0 / min_turning_radius);
  ...
  //  5: 倒车 + 右转
  projections.clear();
  projections.emplace_back( d_dist,  0.0,   0.0,        TurnDirection::FORWARD);    // 前进直线
  projections.emplace_back( d_x,     d_y,   increments,  TurnDirection::LEFT);      // 前进左转
  projections.emplace_back( d_x,    -d_y,  -increments,  TurnDirection::RIGHT);     // 前进右转
  projections.emplace_back(-d_dist,  0.0,   0.0,        TurnDirection::REVERSE);    // 倒车直线
  projections.emplace_back(-d_x,     d_y,  -increments,  TurnDirection::REV_LEFT);  // 倒车左转
  projections.emplace_back(-d_x,    -d_y,   increments,  TurnDirection::REV_RIGHT); // 倒车右转

  // 为每个运动原语 + 每个角度 bin，预计算坐标增量
  delta_xs.resize(projections.size());
  delta_ys.resize(projections.size());
  trig_values.resize(num_angle_quantization);
  ...
  for (unsigned int i = 0; i != projections.size(); ++i)
  {
    ...
    for (unsigned int j = 0; j != num_angle_quantization; ++j)
    {
      const double cos_theta = std::cos(bin_size * static_cast<double>(j));
      const double sin_theta = std::sin(bin_size * static_cast<double>(j));
  ...
  for (unsigned int i = 0; i != projections.size(); ++i)
  {
    ...
    if (turn_dir != TurnDirection::FORWARD && turn_dir != TurnDirection::REVERSE)
    {
      const double arc_angle = projections[i].theta_ * bin_size;        // 弧度
      const double turning_r = 0.5 * d_dist / std::sin(arc_angle * 0.5);
      travel_costs[i]        = static_cast<float>(turning_r * arc_angle);
    }
    else
    {
      travel_costs[i] = static_cast<float>(d_dist);
    }
  }
}
